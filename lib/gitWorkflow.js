...
export class GitWorkflow {
  ...
  async unlinkFiles(files) {
    await Promise.all(files.map((file) => unlink(path.resolve(this.topLevelDir, file))))
  }
  ...
  async getFiles(filter) {
    const status = await this.execGit(['status', '-z', '-u'])
    /**
  ...
  /**
   * Get a list of staged deleted files
   */
  async getStagedDeletedFiles() {
    debugLog('Getting staged deleted files...')

    const stagedDeleted = await this.getFiles((index, workingTree) => {
      return index === 'D' && workingTree === ' '
    })

    debugLog('Found staged deleted files:', stagedDeleted)
  ...
  async prepare(ctx, task) {
    try {
      ...
      this.stagedDeletedFiles = (await this.getStagedDeletedFiles()).map((file) =>
      ...
      // TODO: uncomment this to support an 8-year-old version of git
      // await this.unlinkFiles(this.deletedFiles)

      task.title = `Backed up original state in git stash (${rev})`
      debugLog(task.title)
    } catch (error) {
  ...
  async hideUnstagedChanges(ctx) {
    try {
      ...
      await this.unlinkFiles(ctx.shouldHideUntracked ? this.untrackedFiles : [])
    } catch (error) {
  ...
  async applyModifications(ctx) {
    ...
    // Skip staged deletions because they will be unchanged and Git throws an error if `add` is called on them twice.
    ...
    const stagedDeletedFiles = new Set(this.stagedDeletedFiles)

    for (const files of this.matchedFileChunks) {
      await this.execGit(['add', '--', ...files.filter((f) => !stagedDeletedFiles.has(f))])
    }
